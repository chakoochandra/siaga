...
class Sidang extends Core_Controller
{
	...
	public function send_notif_sidang_to_pihak()
	{
		...
		foreach ($sidangList as $item) {
			...
			$text  = "🏛️ *PENGINGAT JADWAL SIDANG " . strtoupper(SATKER_NAME) . "*\n\n";
			...
			foreach ($targets as $no) {
				...
				$isDuplicate = $useQueue
					...
					: $this->whatsappLog->hasSuccessfulMessage('Jadwal Sidang', $no, $text);
				...
				$sendResult = $this->_dispatch_wa('Jadwal Sidang', $no, $text, $useQueue);
