	...
	function switch_input($type, $field)
	{
		switch ($type) {
			...
			case 'form_info':
				...
				return warning_message($title . $message, 'info', $class);
			case 'form_checkbox':
				$html = '';
			...
			case 'form_dropzone':
				$options = [
					...
					'labelText' => isset($field['labelText']) ? $field['labelText'] : 'Tarik file ke sini',
			...
			case 'form_password':
				return get_instance()->load->view('widgets/password_input', ['field' => $field], true);
			default:
...
if (!function_exists('my_form')) {
	function my_form($data, $options = '', $attributes = [])
	{
		...
		$backClass = isset($options['backButtonClass']) ? $options['backButtonClass'] : 'btn btn-outline-warning btn-back m-1';
		$html .= $showBtnBack ? anchor($options['backUrl'], '<span class="fa fa-chevron-left" aria-hidden="true"></span> ' . ($isConfirmation ? 'Tidak' : 'Kembali'), ['class' => $backClass, 'style' => 'width: 100px;']) : '';
...
if (!function_exists('form_dropzone')) {
	 ...
	 * @return	string
	 */
	function form_dropzone($options = [])
	{
		$ci = &get_instance();
		$defaults = [
