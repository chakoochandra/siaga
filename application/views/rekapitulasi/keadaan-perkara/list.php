...
<script type="text/javascript">
	$(document).ready(function() {
		...
		const show_km = '<?php echo json_encode(['belum_ada_edoc', 'tunggakan', 'tunggakan_bulan_lalu', 'sisa_bulan_lalu', 'sudah_ada_edoc', 'belum_anonimasi', 'belum_minutasi', 'cabut', 'dikabulkan', 'ditolak', 'tidak_diterima', 'digugurkan', 'dicoret', 'sisa_bulan_ini',  'jumlah_semua',  'lain_lain',  'damai', 'minutasi', 'belum_pertimbangan_hukum', 'jumlah_bulan_ini', 'belum_ada_phs']) ?>';
		...
		const show_pendaftaran = '<?php echo json_encode(['sisa_bulan_lalu', 'terima', 'jumlah_bulan_ini', 'sisa_bulan_ini', 'tunggakan', 'tunggakan_bulan_lalu', 'masuk_hari_ini', 'belum_ada_gugatan', 'ecourt_hari_ini', 'belum_ada_pmh', 'belum_ada_phs']) ?>';
		const urlList = '<?php echo base_url("rekapitulasi/keadaanperkara/get_list/{$this->uri->segment(4)}/{$this->uri->segment(5)}/{$this->uri->segment(6)}/{$this->uri->segment(7)}/{$this->uri->segment(8)}/{$this->uri->segment(9)}") ?>';
		...
		initDataTable("#table-perkara", {
			...
			columns: [{
				...
				},
				{
					...
					className: "dt-center",
					visible: show_putusan.includes(status) || status.includes("putus") || status.includes("redaksi") || status.includes("dirput") || status.includes("published"),
					render: function(data, type, row) {
				...
				},
				{
					...
					visible: show_putusan.includes(status) || status.includes("putus") || status.includes("redaksi") || status.includes("published") || status.startsWith("dk_") || status.startsWith("alasan_") || status.includes("dirput"),
					render: function(data, type, row) {
