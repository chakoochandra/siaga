<div class="card leaves">
	...
	<div class="card-body">
		<div class="table-responsive">
			<table id="table-banding" class="display">
				<thead>
					<tr>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		function createUrl(year, month, data, type) {
			return data <= 0 ? `<span class="text-muted">${data}</span>` : `<a href="${'<?php echo base_url('rekapitulasi/banding/perkara_list/') ?>'}${type}/${year}/${month}" class="btn-modal">${data}</a>`;
		}

		table = initDataTable('#table-banding', {
			title: "<?php echo $title ?>",
			// scroller: false,
			// ordering: false,
			showSearchField: false,
			ajax: {
				url: "<?php echo base_url("rekapitulasi/banding/get_statistic") ?>/",
				data: function (d) {
					d['selectedYear'] = $(`.table-banding_datepicker input[type="text"]`).val();
			...
			footerCallback: function (row, data, start, end, display) {
				...
				for (let i = 2; i <= 9; i++) {
					...
					$(api.column(i).footer()).html(createUrl($(`.table-banding_datepicker input[type="text"]`).val(), 0, total, typePutus[i - 2]));
					temp += total;
				}
