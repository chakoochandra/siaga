 ...
 */
function applyGradientsToSelector(selector, dataAttribute = "data-gradient-index") {
	const elements = document.querySelectorAll(selector);
	if (elements.length === 0) return;

	elements.forEach((element, index) => {
		// Skip if already has a gradient applied
		...
		element.style.setProperty("textShadow", "0 1px 2px rgba(0,0,0,0.2)", "important");
...
function observeTableHeaders() {
	...
	tableHeaders.forEach((thead, index) => {
		// Skip if this thead already has a gradient applied
		const firstRow = thead.querySelector("tr");
		if (firstRow && firstRow.hasAttribute("data-table-gradient-index")) {
			return; // already colored, skip
		}

		// Generate ONE gradient per thead
		const gradient = window.GradientGenerator.generate();
		...
		rows.forEach((row, rowIndex) => {
			...
			row.style.setProperty("textShadow", "0 1px 2px rgba(0,0,0,0.2)", "important");
			row.style.setProperty("transition", "background 0.3s ease", "important");
